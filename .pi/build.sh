...
# docker buildx build --tag cns --file .pi/Dockerfile --platform linux/arm64 --cache-from cns:cache --cache-to cns:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar cns
...
sudo bash -c "echo cns > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/cns.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/cns.img
